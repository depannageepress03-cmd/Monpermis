...
import { Chapter, serializeModule } from '../models/Chapter.js'
...
router.post('/chapters', async (req, res) => {
  try {
    ...
    const chapter = await Chapter.create({ name, order: count, published: false })
...
router.patch('/chapters/:chapterId', async (req, res) => {
  try {
    ...
    if (req.body.published !== undefined) {
      ...
      // Un chapitre publié doit exposer ses cours côté apprenant
...
router.post('/chapters/reorder', async (req, res) => {
  try {
    ...
    const updated = await Chapter.find().sort({ order: 1, createdAt: 1 })
    res.json({
      success: true,
...
router.post('/chapters/:chapterId/duplicate', async (req, res) => {
  try {
    const source = await Chapter.findById(req.params.chapterId)
    if (!source) {
      return res.status(404).json({ success: false, error: 'Chapitre introuvable' })
    }

    const count = await Chapter.countDocuments()
    const duplicate = await Chapter.create({
